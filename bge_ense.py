...
RUN_TAGS  = ['bgeL_ep8_lr1e5', 'bgeL_ep10_lr1e5', 'bgeL_ep8_lr2e5', 'bgeL_ep10_lr2e5']
...
for tag in RUN_TAGS:
    ...
    score_mean  = compute_dual_f1(all_df, apply_label_hierarchy(task_logits))['mean']
    run_scores.append(score_mean)

run_scores = np.array(run_scores)
print('Seed-noise floor (bge-large OOF-Mean):')
for tag, score in zip(RUN_TAGS, run_scores):
    print(f'  {tag:<18} -> Mean Score: {score:.4f}')
print(f'  mean {run_scores.mean():.4f} | best {run_scores.max():.4f}')
...
print(f"  vs best single seed Mean {run_scores.max():.4f}  ->  delta {oof_score['mean'] - run_scores.max():+.4f}")
...
out_path = os.path.join(BASE_DIR, 'submission_bge_large_ensemble.csv')
submission.to_csv(out_path, index=False, encoding='utf-8')
print(f'\nsubmission written: {out_path}  (format-validated)')

anchor_path = os.path.join(BASE_DIR, 'out_bgeL_ep10_lr2e5', 'submission_bgeL_ep10_lr2e5.csv')
if os.path.exists(anchor_path):
    ...
    print(f'  rows changed vs best single model anchor: {changed_rows}')
